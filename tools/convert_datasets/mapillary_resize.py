...
def main():
    args = parse_args()
    mmengine.utils.mkdir_or_exist(args.dst_image_folder)
    mmengine.utils.mkdir_or_exist(args.dst_label_folder)
    imgs = []
    for filename in mmengine.utils.scandir(args.image_folder, suffix=".jpg"):
    ...
    if args.nproc > 1:
        mmengine.utils.track_parallel_progress(resize_half, tasks, args.nproc)
    else:
        mmengine.utils.track_progress(resize_half, tasks)
