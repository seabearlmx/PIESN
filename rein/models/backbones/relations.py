...
class Relations(nn.Module):
    ...
    def create_model(self):
        self.Proto = []
        self.Differ = []
        self.inter_relation_matrix = nn.Parameter(torch.empty([24, 19, 19]))
        self.inter_relation_matrix.requires_grad = False

        self.intra_relation_matrix_layer = nn.Parameter(torch.zeros([24, 19, 1024]))
        self.intra_relation_matrix_layer.requires_grad = False

        for i in range(24):
            self.Proto.append(prototype_dist_estimator(feature_num=1024))
    ...
    def inter_class_relation_calib_matrix(self, feat, proto, gt, idx):
        # ----------------- get_class_feat -------------------------------
        feats = feat
        cls_feat = torch.zeros(19, 1024).cuda()
        Bh, Ch, Hh, Wh = feats.size()
        src_mask = F.interpolate(gt.unsqueeze(1).float(), size=(Hh, Wh), mode='nearest').squeeze(
            0).long()
        src_mask = src_mask.contiguous().view(Bh * Hh * Wh, )
    ...
    def forward(
        ...
        new_x = att_x.contiguous().view(B, C, H*W)
        if has_cls_token:
